 ...
 * @subpackage Database
 */
class dbc_Database_MySQL_Result {
	/**#@+
	 * @ignore
	 * @internal
	 */
	...
	public function rewind() {
		@mysql_data_seek($this->_rResult, 0);
	}
	...
	public function first($bAssoc_ = true) {
		if (! $this->_bFirst) @mysql_data_seek($this->_rResult, 0);
	...
	public function next($bAssoc_ = true) {
		$this->_bFirst = false;
		return $bAssoc_ ? mysql_fetch_assoc($this->_rResult) : mysql_fetch_row($this->_rResult);
	}
	...
	public function all($bAssoc_ = true) {
		$aRecords = array($this->first($bAssoc_));
		while(end($aRecords) !== false) $aRecords[] = $this->next($bAssoc_);
		array_pop($aRecords);
		return $aRecords;
	}
	...
	function affectedRows() {
		return mysql_affected_rows($this->_oDatabase->getConnection());
	}
	...
	function getInsertId(){
		return mysql_insert_id($this->_oDatabase->getConnection());
	}
