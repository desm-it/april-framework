 ...
 * @subpackage Database
 */
class dbc_Database_MySQLi_Director extends dbc_Database_Queryable {
	/**#@+
	 * @ignore
	 * @internal
	 */
	...
	protected function __construct(dbc_Config_MySQL $oConfig_, dbc_Config_MySQL $oConfigSlave_ = NULL) {
		$this->_oDatabaseMaster = dbc_Database_MySQLi::getInstance($oConfig_, 'master');
		$aHosts = $oConfig_->get('host');
		if (isset($aHosts['slave'])) {
			$this->_oDatabaseSlave = dbc_Database_MySQLi::getInstance($oConfig_, 'slave');
		} else if (! is_null($oConfigSlave_)) {
			$this->_oDatabaseSlave = dbc_Database_MySQLi::getInstance($oConfigSlave_, 'master');
	...
	static public function getInstance(dbc_Config_MySQL $oConfig_, dbc_Config_MySQL $oConfigSlave_ = NULL) {
		...
		return new dbc_Database_MySQLi_Director($oConfig_, $oConfigSlave_);
	}	
