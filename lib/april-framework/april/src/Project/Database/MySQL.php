...
class MySQL extends dbc_Database_Queryable {
	/**#@+
	 * @ignore
	 * @internal
	 */
	...
	private function __construct(dbc_Config_MySQL $oConfig_, $sType_ = 'master') {
		...
		if (! isset($aHosts[$sType_])) throw new Exception("host type '{$sType_}' not found");
		...
		$this->_aOptions['charset'] = strtolower( $oConfig_->get('charset') ? $oConfig_->get('charset') : 'utf8mb4' );
	 ...
	 *
	 * <code>
	 * $oDb = dbc_Database_MySQL::getInstance($oDbConf, 'master');
	 * $oResult = $oDb->execureQuery("
	 * 	SELECT *
	 * 	FROM dikkenegerin
	 * 	WHERE categorie='%s'
	...
	public function executeQuery($sQuery_ /*, $mArg1, $mArg2 ... $mArgN */) {
		...
		if (func_num_args() > 1) {
			...
			for ($i = 1; $i < count($mArgs_); $i++) {
				$mArgs_[$i] = mysql_real_escape_string($mArgs_[$i], $this->_rConnection);
			}
		...
		if (!$this->_bDaemonMode) self::$_aQueries[] = $sQuery_;

		dbc_Logger::debug("MySQL query executed: " . htmlentities($sQuery_));
		return new dbc_Database_MySQL_Result($this, $sQuery_, mysql_query($sQuery_, $this->_rConnection));
	}
	...
	public function executeUpdateOrInsert( $sTable_, $sKey_, $aFieldValues_, $sKeyField_ ='id' ) {
		...
		$q='';
		foreach($aFieldValues_ as $k=>$v) {
			if( $q!='' ) $q.=',';
			$q.="`$k`='".mysql_real_escape_string($v, $this->_rConnection)."'";
		}
		...
		if( $sKey_ && $this->executeOneQueryAndReturnValue("SELECT `$sKeyField_` FROM `$sTable_` WHERE `$sKeyField_`='$sKey_'")) {
			...
			$q = "UPDATE `$sTable_` SET $q WHERE `$sKeyField_`='$sKey_'";
		} else {
			// Insert new record
			if( $sKey_ && !isset($aFieldValues_[$sKeyField_])) {
				$q = "`$sKeyField_`='".mysql_real_escape_string($sKey_, $this->_rConnection)."',".$q;
			}
			$q = "INSERT INTO `$sTable_` SET ".$q;
		}
	...
	private function __do_escape(& $sValue_, $mKey_, & $rConnection_) {
		$sValue_ = mysql_real_escape_string($sValue_, $rConnection_);
	}
	...
	public function connect() {
		// YYY: Patched 4th parameter, bool $new_link, to use the bDaemonMode so (multithreaded) daemon scripts
		//		always get a new link when they ask for it.
		$this->_rConnection = @mysql_connect(
			$this->_aOptions['host'],
			$this->_aOptions['username'],
			$this->_aOptions['password'],
			$this->_bDaemonMode // new connection on every attempt
		);
		if ($this->_rConnection !== false) {
            if ($this->_aOptions['charset'] != 'latin1') { /* latin1 = default ! */
    			@mysql_query("SET NAMES '{$this->_aOptions['charset']}'", $this->_rConnection); /* sja */
    			
    			
    			//if($_SERVER['REMOTE_ADDR'] == '213.207.92.195'){
	  //  			@mysql_query("SET character_set_results = 'utf8mb4', character_set_client = 'utf8mb4', character_set_connection = 'utf8mb4', character_set_database = 'utf8mb4', character_set_server = 'utf8mb4'", //$this->_rConnection);
    			
    			//}
    			
				self::$_aQueries[] = "SET NAMES '{$this->_aOptions['charset']}'";
				dbc_Logger::debug("database chatset set to {$this->_aOptions['charset']}");
            }
			if (@mysql_select_db($this->_aOptions['database'], $this->_rConnection) == false) {
	...
	public function selectDatabase($sDatabase_) {
		...
		$bResult = @mysql_select_db($sDatabase_, $this->_rConnection);
	...
	public function isConnected() {
		if (
			$this->_rConnection == false ||
			!@mysql_ping($this->_rConnection)
		) return false;

		if ( $this->_bDaemonMode ) {
			$bResult = mysql_query( 'SELECT 1', $this->_rConnection );
	...
	public function getErrorNo() {
		return @mysql_errno($this->_rConnection);
	}
	...
	public function getError() {
		return @mysql_error($this->_rConnection);
	}
	...
	/**
	 * Preferred method of receiving a {@link dbc_Database_MySQL} instance.
	 * @return dbc_Database_MySQL Returns a dbc_Database_MySQL instance with the requested configuration and host type.
	...
	static public function getInstance(dbc_Config_MySQL $oConfig_, $sType_ = 'master') {
		...
		return new dbc_Database_MySQL($oConfig_, $sType_);
	}
